...
export default function Chat() {
  ...
  const send = async (text) => {
    ...
    setSending(true);
    try {
      const res = await sendChat({ session_id: sessionId, message: msg });
      ...
      <div className="bg-white border border-[#E5E1D8] rounded-2xl overflow-hidden flex flex-col h-[640px]">
        <div className="bg-[#5A7059] text-white px-6 py-4 flex items-center gap-3" data-testid="chat-header">
          ...
          <div>
            ...
            <p className="text-xs text-white/70">Compassionate guidance · Not legal/clinical advice</p>
          </div>
        </div>
        ...
        >
          <textarea
            ...
            rows={1}
            placeholder="Tell Anchor what's heavy tonight…"
